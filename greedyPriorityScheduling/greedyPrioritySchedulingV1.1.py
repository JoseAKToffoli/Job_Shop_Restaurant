...
def getPivot (L, prevPivot):
    ...
    TOT of the previous pivot. In case of multiple in-
    finite priorities, we take the order that was fir-
    st requested to be of highest priority. 
    ...
    for i in range(len(indicesOfMaxP) - 1):
        if L[indicesOfMaxP[i+1]].TTC <= prevPivot.TOT and L[indicesOfMaxP[i+1]].TTC > p.TTC:
            p = L[indicesOfMaxP[i+1]]
